...
void vtkMFCRenderView::OnDraw(CDC* pDC)
{
  ...
  vtkWin32OpenGLRenderWindow* renWin = vtkWin32OpenGLRenderWindow::SafeDownCast(
    this->ImageViewer->GetRenderWindow());

  if (this->Interactor && !this->Interactor->GetInitialized())
    {
    this->Interactor->SetRenderWindow(this->ImageViewer->GetRenderWindow());
    WNDPROC OldProc = (WNDPROC) GetWindowLongPtr(this->m_hWnd, GWLP_WNDPROC);
    this->Interactor->Initialize();
    SetWindowLongPtr(this->m_hWnd, GWLP_WNDPROC, (LONG_PTR) OldProc);
    }
  ...
  if (pDC->IsPrinting())
    ...
    BeginWaitCursor();
    memcpy(size,this->ImageViewer->GetSize(),sizeof(int)*2);
    ...
    pDC->DPtoLP(rcDestLP);
    int DPI = this->ImageViewer->GetRenderWindow()->GetDPI();
    
    renWin->SetupMemoryRendering(
      rcDest.right/scale,
      rcDest.top/scale,
      pDC->m_hAttribDC);
...
int vtkMFCRenderView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
  ...
  this->ImageViewer->SetParentId(lpCreateStruct->hwndParent);
  this->ImageViewer->SetWindowId(this->m_hWnd);
...
void vtkMFCRenderView::OnInitialUpdate()
{
  ...
  // Reset camera on first update so it shows the whole image:
  //
  vtkRenderer* ren = this->ImageViewer->GetRenderer();
  if (ren && this->ImageViewer->GetInput())
    {
    ren->ResetCamera();
    }
}
